...
func _handle_new_player_ready(new_id: int):
	...
	if new_id != 1:
		...
		spawn_player_on_all.rpc_id(new_id, 1) # Envoyer Host
		if is_voting_phase:
...
func _on_player_disconnected(id: int):
	...
	if multiplayer.is_server():
		# Le serveur dit à tout le monde (y compris lui-même via call_local) de supprimer le joueur
...
func despawn_player_on_all(id: int):
	var p = players_root.get_node_or_null("Player_" + str(id))
	if p: p.queue_free()
